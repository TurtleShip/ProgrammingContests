...
int HouseBuilding::getMinimum(vector <string> area) {
	...
	for(int curH = 0; curH < 9; curH++) {
		...
		for(int i=0; i < N; i++) {
			for(int j=0; j < M; j++) {
				int num = area[i][j] - '0';
				if( minH <= num && num <= maxH) continue;
				// if(curH == 4) cout<<num<<endl;
				curRes += min( abs(num - minH), abs(num-maxH));
				// if(curH == 4) cout<<curRes<<"\n"<<endl;
			}
		}
		// if(curH == 4) cout<<res<<endl;
		res = min(curRes , res);
	}
	return res;
