...
long long HyperKnight::countCells(int ia, int ib, int numRows, int numColumns, int k) {
	ll res = 0ll;
    ll a = ll(ia);
    ll b = ll(ib);
    ll dr[8] = {a,a,-a,-a,b,b,-b,-b};
    ll dc[8] = {b,-b,b,-b,a,-a,a,-a};
    ...
    for(int mask = 255; mask >= 0; mask--) {
        ...
        ll curArea = ll(numRows - ndr - pdr) * ll(numColumns - ndc - pdc);
        
        for(int superMask = mask + 1; superMask <= 255; superMask++) {
            if( (superMask & mask) == mask ) curArea -= exactNum[superMask];
        }
        
        exactNum[mask] = curArea;
        if(validMoves == k) res += curArea;
    }
