...
int TomekPhone::minKeystrokes(vector <int> frequencies, vector <int> keySizes) {
    ...
    int K = keySizes.size();
    int K_total = accumulate(keySizes.begin(), keySizes.end(), 0);

    if(K_total < F) return -1;
    
    memset(numTaken, 0, sizeof(numTaken));
    ...
    for(int i=0; i < F; i++) {
        ...
        int minStroke = 55;
        for(int j=0; j < K; j++) {
            if(numTaken[j] < keySizes[j] && (numTaken[j] + 1) < minStroke) {
                minIdx = j;
                minStroke = numTaken[j] + 1;
            }
        }
        // take the key placement that results in minimum placing
        numTaken[minIdx]++;
        res += minStroke * frequencies[i];
    }
