...
vector <int> CheapestRoute::routePrice(vector <int> cellP, vector <int> enterC, vector <int> exitC, int telP) {
    ...
    while(!Q.empty())
    {
        ...
        if(idx == N-1)
        {
            ...
            ans[1] = min(ans[1], move);
            continue;
        }
        
        chk[idx] = true;
        //go left
        if(idx > 0 && cellP[idx-1] != -1 && !chk[idx-1])
        {
            Q.push(idx-1);
        ...
        //go right
        if(idx < N-1 && cellP[idx+1] != -1 && !chk[idx+1])
        {
            Q.push(idx+1);
            Q.push(cost + cellP[idx+1]);
            Q.push(move+1);
        ...
        for(int i=0; i < T; i++)
        {
            if(enterC[i] == idx && cellP[exitC[i]] != -1 && !chk[exitC[i]])
            {
                Q.push(exitC[i]);
