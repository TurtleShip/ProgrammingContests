...
double ConvexPolygon::findArea(vector <int> x, vector <int> y) {
    ...
    REP(i,0,(N-1))
    {
        left += double(x[i] * y[i+1]) * 0.5;
        right += double(y[i] * x[i+1]) * 0.5;
    }
    left += double(x[N-1] * y[0]) * 0.5;
    right += double(y[N-1] * x[0]) * 0.5;
    
    /*
     We don't know if vertex are given in counterclockwise or the other.
     so return absolute value 
     */
