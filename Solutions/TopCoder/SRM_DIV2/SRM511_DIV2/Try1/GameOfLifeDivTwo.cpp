...
string GameOfLifeDivTwo::theSimulation(string init, int T){
    ...
    for(int i=0; i < T; i++)
    {
        ...
        if(prev[1] == '1' && prev[N-1] == '1') next[0] = '1';
        if(prev[1] == '0' && prev[N-1] == '0') next[0] = '0';
        if(prev[0] == '1' && prev[N-2] == '1') next[N-1] = '1';
        if(prev[0] == '0' && prev[N-2] == '0') next[N-1] = '0';
        for(int j=1; j < N-1; j++)
        {
            if(prev[j-1] == '1' && prev[j+1] == '1') next[j] = '1';
            if(prev[j-1] == '0' && prev[j+1] == '0') next[j] = '0';
        }
        prev = next;
    }
    return next;
