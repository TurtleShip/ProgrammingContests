...
int HexagonPuzzle::theCount(vector <string> board) {
    ...
    memset(parts, false, sizeof(parts));
    REP(i,0,N)
    REP(j,0,i+1)
        ori[i][j] = (board[i][j] == '.');
...
int rec(int curR, int curC)
{
    if(chk[curR][curC] || !parts[curR][curC]) return 0;
    //cout<<"rec("<<curR<<" , "<<curC<<")"<<endl;
    chk[curR][curC] = true;
    int dr[3] = {0,1,1};
    int dc[3] = {1,0,1};
    int res = 1;
    ...
    //cout<<"rec("<<curR<<" , "<<curC<<") => res : "<<res<<endl;

    return res;
}

bool isPart(int r, int c)
{
    if(!ori[r][c]) return false;
