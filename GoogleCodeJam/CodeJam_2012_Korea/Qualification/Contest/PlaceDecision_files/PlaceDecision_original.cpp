        ...
        if(valid)
        {
            ...
            for(int i=0; i < P; i++)
            {
                ...
                while(!Q.empty())
                {
                    ...
                    {
                        if(j == start) continue;
                        int newDist = curDist + frds[i].second * dist[curLoc][j];
                        //if((frdDist[i][curLoc][j] == 0 || newDist < frdDist[i][curLoc][j]) && dist[curLoc][j] != -1)
                        if( (frdDist[i][j] == 0 || newDist < frdDist[i][j]) && dist[curLoc][j] != -1 ) 
                        {
                            //frdDist[i][curLoc][j] = frdDist[i][j][curLoc] = newDist;
                            frdDist[i][j] = newDist;
                            Q.push(j);
                            Q.push(newDist);
                        }
