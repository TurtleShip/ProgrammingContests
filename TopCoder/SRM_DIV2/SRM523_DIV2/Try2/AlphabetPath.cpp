...
string AlphabetPath::doesItExist(vector <string> letterMaze) {
    ...
    {
        bool found = false;
        for(int r=0; r < letterMaze.size() && !found; r++)
            for(int c=0; c < letterMaze[0].size() && !found; c++)
            {
                if(letterMaze[r][c] == ch)
                {
    ...
    REP(i,1,pos.size())
    {
        if( (abs(pos[i].first - pos[i-1].first) + abs(pos[i].second - pos[i-1].second)) != 1)
            return "NO";
    }
    return "YES";
}
