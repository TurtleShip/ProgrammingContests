...
/*
 //This approach works for small strings, but resulted in exceeding memory limit for large strings
string getN(int idx)
{
    string& str = A[idx];
...
/*
 return 1 if A[id][index] == '1'. else return 0
 */
int hasOne(int id, ll index)
{
    if(id < K)
    {
        if(first[id][(int)index] == '1') return 1;
        return 0;
    }
    else
    {
        ...
        while(true)
        {
            //cout<<"accesssing "<<(id - m*K -1)<<endl;
            if(index < len[id - m*K -1]) break;
            
            index -= len[id - m*K - 1];
            m++;
        }
      //  cout<<"("<<id<<" , "<<index<<") is finished"<<endl;
        return hasOne(id-m*K-1 , index);
    }
}

//exceeds limit of long long
ll fillLen(int id)
{
    ll& res = len[id];
...
int MagicalGirlLevelThreeDivTwo::theCount(vector <string> _first, int n, long long lo, long long hi) {
