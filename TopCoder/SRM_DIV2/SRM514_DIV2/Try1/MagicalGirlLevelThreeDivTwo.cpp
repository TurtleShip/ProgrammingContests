...
int MagicalGirlLevelThreeDivTwo::theCount(vector <string> first, int n, long long lo, long long hi) {
	...
	for(int i=K; i<=n; i++)
	{
        ...
        while(pos >= 0)
        {
            count[i] += count[pos];
            pos -= K;
            if(count[i] > hi) break; //very crucical!!
            /*
             since A[i] = A[i-1] + ... if A[i] is longer than hi,
             any A[j] (i<j) will start with A[i] , and hi will be in there.
             This applies to A[n] which will start with A[i] and its "hi"th number will be in A[i]
             */
        }
    }
    //done calculating length of each count
//    cout<<"Done calculating"<<endl;
  //  for(int i=0; i <= n; i++)
    //    cout<<"count["<<i<<"] : "<<count[i]<<endl;
    //Figure out all A[n][idx]
    for(ll idx = lo; idx <= hi; idx++)
    {
        //A[n][idx] is from first[pos][loc]
        int pos = n;
        ll loc = idx;
       // cout<<"idx : "<<idx<<endl;
        while(pos >= K)
        ...
        //cout<<"idx : "<<idx<<" , pos : "<<pos<<" , loc : "<<loc<<endl;
        if(first[pos][loc] == '1') power++;
    }
    return power;
}
