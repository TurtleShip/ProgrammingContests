...
int MysteriousRestaurant::maxDays(vector <string> prices, int budget){
    ...
    for(int i=0; i < N; i++)
        for(int j=0; j < M; j++)
         chart[i][j] = getNum(prices[i][j]);

    
    while(res)
    {
        //find optimal choice for each day
        for(int i=0; i < week; i++)
        {
            ...
            choice[i] = minC;
        // cout<<"i : "<<i<<" : "<<choice[i]<<endl;
        }
  
        //Go through every day until you are broke
        int tempD = 0;
        int tempB = budget;
        while(tempD < res)
        {
            tempB -= chart[tempD][choice[tempD % 7]];
            if(tempB < 0) break;
            tempD++;
        }
        if(tempD == res) break;
