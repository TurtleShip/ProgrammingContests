...
int GogoXReimuHakurai::solve(vector <string> choices) {
    ...
    /* road[i]
     = 0 if node i is not in any valid path
     = 1 if node 0 can reach node i
     = 2 if ( node 0 can reach node i ) and ( node i can reach node (N-1) )
     */
    ...
    int V = 0, E = 0;
    V = count(road, road + N, 2);
    
    REP(i,0,N)
    REP(j,0,N)
    if(choices[i][j] == 'Y' && road[i] == 2 && road[j] == 2)
        E++;
